...
(defcfg/defsimconfig [[num-k-snipes       50    long   [1 500]     ["-N" "Size of k-snipe subpopulation" :parse-fn #(Long. %)]]
                      ...
                      [popenv             nil   free-agent.SimConfig.PopEnv false]])
...
;; note underscores
(defn is-k-snipe? [s] (instance? free_agent.SimConfig.KSnipe s))
(defn is-r-snipe? [s] (instance? free_agent.SimConfig.RSnipe s))
                              ...
                              (let [len (count snipes)]              ; convert to key-value pairs where value is a snipe
                                (if (= len 1)
                                  ...
                                  (let [mover (nth snipes (ran/rand-idx rng len))] ; randomly select one to move
                                    (into {coords mover} (map (fn [snipe] {[(get-x snipe) (get-y snipe)] snipe}) ; and make others "move" to current loc
...
(defn eat-if-appetizing 
  ...
  (let [[experienced-snipe appetizing?] (perceive-mushroom snipe mush)]
    (if appetizing?
      (do
        (set-energy! experienced-snipe  ; FOR NON-FNL TEST (update experienced-snipe :energy add-to-energy max-energy (:nutrition mush)) 
                        (add-to-energy (get-energy experienced-snipe)
                                       max-energy 
                                       (:nutrition mush)))
...
(defn snipes-eat!
  ...
  (let [{:keys [env-width env-height max-energy]} cfg-data
        ...
        snipes-plus-eaten? (for [snipe snipes    ; returns only snipes on mushrooms
                                 :let [x (get-x snipe) ; FOR NON-FNL TEST {:keys [x y]} snipe
                                       y (get-y snipe) ; FOR NON-FNL TEST
...
(defn snipes-die!
  ...
  (let [{:keys [env-width env-height]} cfg-data
        new-snipe-field snipe-field  ; FOR NON-FNL TEST (ObjectGrid2D. env-width env-height) 
        live-snipes (remove #(<= (get-energy %) 0) (.elements snipe-field))]
    (.clear new-snipe-field) ; FOR NON-FNL TEST
    (doseq [snipe live-snipes]
      (.set new-snipe-field (get-x snipe) (get-y snipe) snipe))
    new-snipe-field))
...
(defn snipes-reproduce!
  ...
  (let [{:keys [env-width env-height birth-threshold birth-cost]} cfg-data
    ...
    (doseq [snipe old-snipes]
      (when (>= (get-energy snipe) birth-threshold)
        (set-energy! snipe (- (get-energy snipe) birth-cost)) ; FOR NON-FNL TEST
        (.set new-snipe-field (get-x snipe) (get-y snipe)  ; replace with energy reduced due to birth
