...
(defn curr-popenv
  "Convenience function to return the popenv."
  [^Sim sim]
  (:popenv @(.simData sim)))
...
(defn cleanup
  "Tasks to be performed by finish(), but separated out so that
  it can be called separately if desired."
  [^Sim this]
  (let [^SimData sim-data$ (.simData this)
        sim-data @sim-data$
...
(defn run-sim
  ...
  call the simulation code for a single step."
